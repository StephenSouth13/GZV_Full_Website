...
const defaultNav: NavItem[] = [
  ...
  { href: "/dong-hanh", label_vi: "Đồng hành", label_en: "Partners", sort_order: 60, is_visible: true, is_page_enabled: true },
...
function SiteContentManager() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setSelectedSectionKey(sectionsResult.data?.[0]?.section_key || "projects")
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 md:p-6">
      <div>
        <p className="text-sm font-black uppercase tracking-[0.28em] text-blue-600">Website CMS</p>
        <h1 className="mt-2 text-3xl font-black text-slate-900 dark:text-white">Nội dung website</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">Chỉnh header, trang, section trang chủ, footer, floating chat và loading.</p>
      </div>

      <Tabs defaultValue="home" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2 gap-1 md:grid-cols-8">
          ...
          <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
            <Card>
              ...
              <CardContent className="space-y-2">
                {[...new Set([...pages.map((p) => p.slug), ...pageBlocks.map((b) => b.page_slug), "dao-tao"])].map((slug) => (
                  <button key={slug} onClick={() => setBuilderSlug(slug)} className={`w-full rounded-lg px-3 py-2 text-left text-sm font-bold ${builderSlug === slug ? "bg-blue-600 text-white" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}>{slug}</button>
                ))}
              </CardContent>
            </Card>
            <Card>
          ...
          <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
            <ListCard title="Section trang chủ">
              {homeSections.map((section) => (
                <button key={section.section_key} onClick={() => setSelectedSectionKey(section.section_key)} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm font-bold transition ${selectedSectionKey === section.section_key ? "bg-blue-600 text-white" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
            ...
            {selectedSection && (
              <Card>
                <CardHeader><CardTitle>Chỉnh section: {selectedSection.title}</CardTitle><CardDescription>Bật tắt, đổi tiêu đề, mô tả, nút và số lượng item trên trang chủ.</CardDescription></CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Button onClick={saveHomeSections} disabled={saving} className="gap-2"><Save className="h-4 w-4" /> Lưu section trang chủ</Button>
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {navItems.map((item, index) => (
                <div key={item.href} className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-[1.2fr_1.2fr_0.7fr_0.7fr_0.7fr] dark:bg-slate-900">
              ...
              <Button onClick={saveNavigation} disabled={saving} className="gap-2"><Save className="h-4 w-4" /> Lưu menu header</Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="pages">
          <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
            <ListCard title="Danh sách trang">
              {pages.map((page) => (
                <button key={page.slug} onClick={() => setSelectedSlug(page.slug)} className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm font-bold transition ${selectedSlug === page.slug ? "bg-blue-600 text-white" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
            ...
            {selectedPage && (
              <Card>
                <CardHeader><CardTitle>Chỉnh trang: {selectedPage.title}</CardTitle><CardDescription>Banner áp dụng cho PageBanner. Nội dung HTML sẽ được chèn thêm cuối trang public.</CardDescription></CardHeader>
                <CardContent className="space-y-5">
                  <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Button onClick={savePage} disabled={saving} className="gap-2"><Save className="h-4 w-4" /> Lưu trang</Button>
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {floating.map((item, index) => (
                <div key={item.action_key} className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-[1fr_1fr_1fr_0.7fr_0.7fr_auto] dark:bg-slate-900">
                  ...
                  <div className="flex items-end gap-2"><Switch checked={item.is_visible} onCheckedChange={(v) => updateFloating(index, { is_visible: v })} /><Button variant="outline" size="icon" onClick={() => setPickerOpen({ floatingIndex: index })}><ImageIcon className="h-4 w-4" /></Button><Button variant="destructive" size="icon" onClick={() => setFloating((rows) => rows.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button></div>
                </div>
              ))}
              <Button variant="outline" onClick={() => setFloating((rows) => [...rows, { action_key: `action-${Date.now()}`, label: "Nút mới", href: "", icon_url: "", action_type: "link", sort_order: rows.length * 10 + 10, is_visible: true }])}><Plus className="mr-2 h-4 w-4" /> Thêm nút</Button>
              <Button onClick={saveFloating} disabled={saving} className="ml-2 gap-2"><Save className="h-4 w-4" /> Lưu floating</Button>
            </CardContent>
          </Card>
        </TabsContent>
