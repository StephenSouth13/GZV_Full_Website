              ...
              {isOpen && (
                ...
                >
                  <div className="border-b border-slate-200 bg-[#050505] px-4 py-3 text-white">
                    ...
                    <p className="mt-1 text-sm font-black uppercase">Kết nối nhanh</p>
                  </div>
                  <div className="grid gap-1 p-2">
                    {visibleActions.map((action) => (
                      <FloatingActionButton key={action.action_key} action={action} onClick={() => openAction(action)} />
                    ))}
                  </div>
                </motion.div>
              )}
...
function FloatingActionButton({ action, onClick }: { action: FloatingAction; onClick: () => void }) {
  const content = (
    <span className="group flex w-full items-center gap-3 border border-transparent px-3 py-3 text-left transition hover:border-[#ed1c24] hover:bg-red-50">
      ...
      <span className="min-w-0">
        ...
        <span className="block truncate text-[11px] font-bold text-slate-500">
          {action.action_type === "chatbot" ? "Mở chatbot" : action.href || "Chưa có link"}
        </span>
      </span>
    </span>
  )
  ...
  if (action.action_type === "link" && action.href) {
    return (
      <Link href={action.href} target="_blank" rel="noopener noreferrer" aria-label={action.label}>
        {content}
      </Link>
    )
  }
  ...
  return (
    <button type="button" onClick={onClick} className="w-full" aria-label={action.label}>
      {content}
    </button>
  )
}
