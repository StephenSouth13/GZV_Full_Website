...
interface MscersTableProps {
  mscers: any[]
  onEdit: (mscer: any) => void
  onDelete: (mscer: any) => void
}

export function MscersTable({ mscers, onEdit, onDelete }: MscersTableProps) {
  return (
    <div className="rounded-[2rem] border border-white/5 bg-gray-900/20 overflow-hidden backdrop-blur-sm">
      <Table>
        <TableHeader className="bg-white/5">
          <TableRow className="border-white/5 hover:bg-transparent">
            <TableHead className="w-[350px] text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 py-6 pl-8">Chuyên gia MSC</TableHead>
        ...
        <TableBody>
          {mscers.length === 0 ? (
            ...
            mscers.map((mscer) => (
              <TableRow key={mscer.id} className="border-white/5 hover:bg-white/[0.02] transition-colors group">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <Avatar className="h-12 w-12 border-2 border-white/10 group-hover:border-blue-500 transition-colors">
                        <AvatarImage src={mscer.avatar_url} className="object-cover" />
                        <AvatarFallback className="bg-gray-800 text-xs font-black">MSC</AvatarFallback>
                      </Avatar>
                      {mscer.is_active && (
                    ...
                    <div>
                      <h4 className="font-bold text-white text-sm group-hover:text-blue-400 transition-colors">{mscer.full_name}</h4>
                      <p className="text-[10px] font-mono text-gray-600 mt-0.5">/{mscer.slug}</p>
                    </div>
                  </div>
                </TableCell>
                ...
                <TableCell>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-gray-300">{mscer.position}</span>
                    <span className="text-[10px] font-black text-blue-500/60 uppercase tracking-tighter">{mscer.company || 'MSC Center'}</span>
                  </div>
                </TableCell>
                ...
                <TableCell className="text-center">
                   <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter ${mscer.is_active ? 'bg-blue-500/10 text-blue-400' : 'bg-gray-800 text-gray-500'}`}>
                      {mscer.is_active ? 'Đang hiển thị' : 'Đang ẩn'}
                   </div>
                </TableCell>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="bg-gray-900 border-white/10 text-white rounded-2xl w-48 p-2 shadow-2xl">
                      ...
                      {mscer.cv_url && (
                        ...
                        >
                          <a href={mscer.cv_url} target="_blank" rel="noreferrer">
